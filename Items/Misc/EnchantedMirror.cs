...
namespace AlchemistNPCReborn.Items.Misc
{
    public class EnchantedMirror : ModItem
    {
		...
		public override bool? UseItem(Player player)/* tModPorter Suggestion: Return null instead of false */
		{
			...
			{
			AlchemistNPCReborn.ppx = player.position.X;
			AlchemistNPCReborn.ppy = player.position.Y;
		...
		{
			if (AlchemistNPCReborn.ppx != 0f)
			{
			RunTeleport(player, new Vector2(AlchemistNPCReborn.ppx, AlchemistNPCReborn.ppy), syncData, false);
			}
			else return;
		}
