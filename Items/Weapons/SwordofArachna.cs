	...
	public class SwordofArachna : ModItem
	{
		...
		public override void OnHitNPC(Player player, NPC target, int damage, float knockback, bool crit)
		{
			if (target.HasBuff(Mod.Find<ModBuff>("Infested").Type))
			{
				...
				vel6 *= 3f;
				int p3 = Projectile.NewProjectile(((Entity) player).GetSource_FromThis((string) null),target.position.X, target.position.Y-30, vel6.X, vel6.Y, 379, damage/2, 0, Main.myPlayer);
