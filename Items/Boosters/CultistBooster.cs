...
namespace AlchemistNPCReborn.Items.Boosters
{
	class CultistBooster : ModItem
	{
        ...
        {
			if (player.GetModPlayer<AlchemistNPCRebornPlayer>().CultistBooster == 0)
			{
				player.GetModPlayer<AlchemistNPCRebornPlayer>().CultistBooster = 1;
				return true;
			}
			if (player.GetModPlayer<AlchemistNPCRebornPlayer>().CultistBooster == 1)
			{
				player.GetModPlayer<AlchemistNPCRebornPlayer>().CultistBooster = 0;
				return true;
			}
