...
namespace AlchemistNPCReborn.Buffs
{
	public class YuiS : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex)
		{
			...
			player.GetModPlayer<AlchemistNPCRebornPlayer>().YuiS = true;
			player.GetModPlayer<AlchemistNPCRebornPlayer>().Yui = false;
			player.buffTime[buffIndex] = 18000;
			bool petProjectileNotSpawned = player.ownedProjectileCounts[ModContent.ProjectileType<Projectiles.Pets.YuiS>()] <= 0;
			if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
			{
				Projectile.NewProjectile(((Entity) player).GetSource_FromThis((string) null),player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, ModContent.ProjectileType<Projectiles.Pets.YuiS>(), 0, 0f, player.whoAmI, 0f, 0f);
			}
			...
			//		{
			//			if (Main.Projectile[j].active && Main.Projectile[j].type == ModContent.ProjectileType<Projectiles.Pets.YuiS>() && Main.Projectile[j].owner == player.whoAmI)
			//			{
