			...
			for (int h = 0; h < 2; h++)
				{
					...
					vel *= 5f;
                    Projectile.NewProjectile(((Entity) this.Projectile).GetSource_FromThis((string) null), Projectile.Center.X, Projectile.Center.Y, vel.X, vel.Y, ModContent.ProjectileType<Chloroshard1>(), Projectile.damage/3, 0, Main.myPlayer);
				}
			return true;
		}
			...
			for (int g = 0; g < 2; g++)
				{
					...
					vel = vel.RotatedBy(rand);
					vel *= 5f;
                    Projectile.NewProjectile(((Entity) target).GetSource_FromThis((string) null), Projectile.Center.X, Projectile.Center.Y, vel.X, vel.Y, ModContent.ProjectileType<Chloroshard1>(), Projectile.damage/3, 0, Main.myPlayer);
			}
		}
	}
}