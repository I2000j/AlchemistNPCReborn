...
{
	public class PF398 : ModProjectile
	{
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			vel *= 0f;
			Projectile.NewProjectile(((Entity) target).GetSource_FromThis((string) null),target.position.X, target.position.Y, vel.X, vel.Y, Mod.Find<ModProjectile>("ExplosionDummyQD").Type, Projectile.damage/2, 0, Main.myPlayer);
		}
		...
		public override void AI()
		{
			...
			else
			{
				...
				for (int k = 0; k < chargeFact + 1; k++)
				{
					...
					Dust dust = Main.dust[Dust.NewDust(pos, 20, 20, Mod.Find<ModDust>("Electrocute").Type, Projectile.velocity.X / 2f,
			...
			for (int i = 0; i < 2; ++i)
			{
				...
				Dust dust = Main.dust[Dust.NewDust(dustPos, 0, 0, Mod.Find<ModDust>("Electrocute").Type, dustVel.X, dustVel.Y, 0, new Color(), 1f)];
				dust.noGravity = true;
				dust.scale = 1.2f;
				...
				dust = Main.dust[Dust.NewDust(Main.player[Projectile.owner].Center + unit * 5f, 0, 0, Mod.Find<ModDust>("Electrocute").Type, unit.X, unit.Y, 0, new Color(), 1f)];
				dust.fadeIn = 0f;
				dust.noGravity = true;
				dust.scale = 0.88f;
			}
			if (Main.rand.Next(5) == 0)
			{
				...
				Dust dust = Main.dust[Dust.NewDust(dustPos + offset - Vector2.One * 4f, 8, 8, Mod.Find<ModDust>("Electrocute").Type, 0.0f, 0.0f, 100, new Color(), 1.5f)];
				dust.velocity = dust.velocity * 0.5f;
				dust.velocity.Y = -Math.Abs(dust.velocity.Y);

				unit = dustPos - Main.player[Projectile.owner].Center;
				unit.Normalize();
				dust = Main.dust[Dust.NewDust(Main.player[Projectile.owner].Center + 55 * unit, 8, 8, Mod.Find<ModDust>("Electrocute").Type, 0.0f, 0.0f, 100, new Color(), 1.5f)];
				dust.velocity = dust.velocity * 0.5f;
				dust.velocity.Y = -Math.Abs(dust.velocity.Y);
			}
			#endregion
