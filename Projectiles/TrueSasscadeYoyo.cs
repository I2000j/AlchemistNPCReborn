...
{
	public class TrueSasscadeYoyo : ModProjectile
	{
			...
			for (int i = 0; i < 200; i++)
            {
                ...
                if (distance < 500f && !target.friendly && target.active)
                {
                    if (Projectile.ai[0] > 60f) // Time in (60 = 1 second) 
                    {
						...
						{
                        Projectile.NewProjectile(((Entity) this.Projectile).GetSource_FromThis((string) null),Projectile.Center.X, Projectile.Center.Y, shootToX, shootToY, ProjectileID.NebulaArcanum, Projectile.damage/3, 0, Main.myPlayer, 0f, 0f);
						}
                        Projectile.ai[0] = 0f;
                    }
                }
