...
{
	public class MagicWandC : ModProjectile
	{
		...
		public void DrawLaser(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 50)
		{
			...
			{
				texture = Request<Texture2D>("AlchemistNPCReborn/Projectiles/MagicWandC", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
			...
			{
				texture = Request<Texture2D>("AlchemistNPCReborn/Projectiles/MagicWandB", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
				#region Draw laser body
				for (float i = transDist; i <= Distance; i += step)
				{
					Color c = Color.White;
			...
			if (Charge == MAX_CHARGE && supercharge >= 180 && hypercharge >= 180)
			{
				texture = Request<Texture2D>("AlchemistNPCReborn/Projectiles/MagicWandA", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
				#region Draw laser body
				for (float i = transDist; i <= Distance; i += step)
				{
					Color c = Color.White;
