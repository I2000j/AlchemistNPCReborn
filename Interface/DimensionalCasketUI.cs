...
using AlchemistNPCReborn.Items.Misc;
using ReLogic.Graphics;
using Microsoft.Xna.Framework.Input;
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;
...
namespace AlchemistNPCReborn.Interface
{
	class DimensionalCasketUI : UIState
	{
		...
		{
			visible = false;
			if (NPC.AnyNPCs(mod.Find<ModNPC>("Alchemist").Type))
			{
				for (k = 0; k < 200; k++)
				{
					NPC npc = Main.npc[k];
					if (Main.npc[k].type == mod.Find<ModNPC>("Alchemist").Type)
					{
		...
		{
			visible = false;
			if (NPC.AnyNPCs(mod.Find<ModNPC>("Brewer").Type))
			{
				for (k = 0; k < 200; k++)
				{
					NPC npc = Main.npc[k];
					if (Main.npc[k].type == mod.Find<ModNPC>("Brewer").Type)
					{
						Brewer.Shop = 1;
						Main.playerInventory = true;
						forcetalk = true;
		...
		{
			visible = false;
			if (NPC.AnyNPCs(mod.Find<ModNPC>("Architect").Type))
			{
				for (k = 0; k < 200; k++)
				{
					NPC npc = Main.npc[k];
					if (Main.npc[k].type == mod.Find<ModNPC>("Architect").Type)
					{
						Architect.Shop = 1;
						Main.playerInventory = true;
						forcetalk = true;
		...
		{
			visible = false;
			if (NPC.AnyNPCs(mod.Find<ModNPC>("Operator").Type))
			{
				for (k = 0; k < 200; k++)
				{
					NPC npc = Main.npc[k];
					if (Main.npc[k].type == mod.Find<ModNPC>("Operator").Type)
					{
						Operator.Shop = 1;
						Main.playerInventory = true;
						forcetalk = true;
		...
		{
			visible = false;
			if (NPC.AnyNPCs(mod.Find<ModNPC>("Jeweler").Type))
			{
				for (k = 0; k < 200; k++)
				{
					NPC npc = Main.npc[k];
					if (Main.npc[k].type == mod.Find<ModNPC>("Jeweler").Type)
					{
						Jeweler.OH = true;
						Jeweler.AS = false;
						Main.playerInventory = true;
		...
		{
			visible = false;
			if (NPC.AnyNPCs(mod.Find<ModNPC>("Young Brewer").Type))
			{
				for (k = 0; k < 200; k++)
				{
					NPC npc = Main.npc[k];
					if (Main.npc[k].type == mod.Find<ModNPC>("Young Brewer").Type)
					{
		...
		{
			visible = false;
			if (NPC.AnyNPCs(mod.Find<ModNPC>("Musician").Type))
			{
				for (k = 0; k < 200; k++)
				{
					NPC npc = Main.npc[k];
					if (Main.npc[k].type == mod.Find<ModNPC>("Musician").Type)
					{
						Musician.S1 = true;
						Musician.S2 = false;
						Musician.S3 = false;
		...
		{
			visible = false;
			if (NPC.AnyNPCs(mod.Find<ModNPC>("Explorer").Type))
			{
				for (k = 0; k < 200; k++)
				{
					NPC npc = Main.npc[k];
					if (Main.npc[k].type == mod.Find<ModNPC>("Explorer").Type)
					{
						Main.playerInventory = true;
						forcetalk = true;
						Main.player[Main.myPlayer].talkNPC = k;
